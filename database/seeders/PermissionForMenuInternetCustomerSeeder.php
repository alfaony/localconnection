...
class PermissionForMenuInternetCustomerSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        try {
            ...
            $itemCustomerInternet = ['index','edit', 'create', 'update', 'show', 'destroy', 'store', 'select2','workflow','dataTableJson','delivery','as_technician','as_finance','closed','editInstalasi','moveRouter'];
            $chatMessage = ['index','store','show','edit'];
             ...
             foreach ($itemCustomerInternet as $method) 
             {
                ...
                if (in_array($method, ['as_technician','complete','closed','editInstalasi','moveRouter'])) 
                {
                    PermissionRole::create(['role_id' => $tecknicianRole->id, 'permission_id' => $permission->id]);
                    continue;
                }
