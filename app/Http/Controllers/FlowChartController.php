...
class FlowChartController extends Controller
{
    ...
    {
        $nodes = FlowNode::where('flow_chart_id', $id)->get();
        $edges = FlowEdge::where('flow_chart_id', $id)->get();
    ...
    {
        $node = FlowNode::create($request->only([
            'flow_chart_id', 'title', 'type', 'x', 'y'
        ]));
    ...
    {
        $node = FlowNode::findOrFail($id);
        $node->update($request->only(['title', 'type', 'x', 'y']));
    ...
    {
        $edge = FlowEdge::create($request->only([
            'flow_chart_id', 'from_node_id', 'to_node_id'
        ]));
