...
class UpdateDailyTasksMITUser extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tasks:update-status-mit-user';
    ...
    public function handle()
    {
        try {
            DB::transaction(function () {
                // Ambil semua DailyTask dengan status 'inReview', user dengan divisi 'MIT', dan end_date sebelum 1 Desember 2024
                $tasks = DailyTask::whereHas('taskStatus', function ($query) {
                    $query->where('name', ParamSchema::INREVIEW);
                })
                ->whereDate('end_date', '<', Carbon::create(2024, 12, 13))
