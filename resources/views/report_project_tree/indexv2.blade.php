...
<div id="accordion">
        ...
        <div id="collapseDashboardProject" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
            <div class="card-body">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-2 text-center">
                        <div class="status-box bg-warning text-white p-2 rounded">
                            ...
                            <h6 class="mt-1" id="doingCount">0</h6>
                    ...
                    <div class="col-md-2 text-center">
                        <div class="status-box bg-info text-white p-2 rounded">
                            ...
                            <h6 class="mt-1" id="inReviewCount">0</h6>
                    ...
                    <div class="col-md-2 text-center">
                        <div class="status-box bg-danger text-white p-2 rounded">
                            ...
                            <h6 class="mt-1" id="notCompleteCount">0</h6>
                ...
                <div class="accordion" id="tasksAccordion">
                    <div class="card">
                        @canAccess('getOverdueTasks', 'project_dashboards')
                    ...
                    <div class="card">                        
                        @canAccess('getUpcomingTasks', 'project_dashboards')
                        <div class="card-header" id="headingUpcoming">
                            <h5 class="mb-0">
                                <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapseUpcoming" aria-expanded="false" aria-controls="collapseUpcoming">
                                    Upcoming Tasks
                                </button>
                            </h5>
                        </div>
    ...
    @canAccess('getVisions', 'project_dashboards')
    <div class="card mt-2">
        <div class="card-header">
            <h5 class="mb-0">
                <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseVisions" aria-expanded="false" aria-controls="collapseVisions">
                    Vision Tracking
                </button>
            </h5>
        </div>
...
<script>
    ...
    {
        const overdueTasksChartContainer = document.getElementById('overdueTasksChart').getContext('2d');
        ...
        const updateDashboard = async () => {
            ...
            const totalCounts = await fetchData(apiEndpoints.totalCounts, params);
            if (totalCounts) {
                document.querySelector('#todoCount').textContent = totalCounts.todo;
                document.querySelector('#doingCount').textContent = totalCounts.doing;
                document.querySelector('#inReviewCount').textContent = totalCounts.in_review;
                document.querySelector('#notCompleteCount').textContent = totalCounts.not_complate;
            }
            ...
            if (overdueTasks) {
                ...
                {
                    overdueTasksChart.destroy();
                }
                
                overdueTasksChart = new Chart(overdueTasksChartContainer, {
                    type: 'bar',
                    data: {
                        labels,
                        datasets: [
                    ...
                    options: 
                    {
                        ...
                        onClick: function (e, elements) {
                            if (elements.length > 0) {
                                ...
                                const userName = labels[index]; // Get the user's name
                                const userId = userIds[index]; // Get the user's ID

                                // Call the function to fetch user tasks dynamically
            ...
            if (upcomingTasks) {
                ...
                {
                    upcomingTasksChart.destroy();
                }

                upcomingTasksChart = new Chart(upcomingTasksChartContainer, {
                    type: 'bar',
                    data: { labels, datasets: [{ label: 'Upcoming Tasks', data }] },
                    options: 
                    {
                        responsive: true,
                        onClick: function (e, elements) {
        ...
        function getStatusIcon(taskStatusName) {
            switch (taskStatusName) {
                ...
                case 'doing':
                    return '<i class="fa fa-hourglass-start"></i> Doing';
                case 'in review':
                    return '<i class="fa fa-eye" style="color: green;"></i> In Review';
                case 'not complete':
                    return '<i class="fa fa-times-circle" style="color: red;"></i> Not Complete';
                case 'complete':
                    return '<i class="fa fa-check" style="color: green;"></i> Complete';
                default:
                    return taskStatusName || '?'; // Fallback to status name or '?' if undefined
            }
        }

        document.querySelector('#filterButton').addEventListener('click', updateDashboard);
    ...
    function fetchUserTasks(userId, userName, filter = 'overdue') 
    {
        ...
        fetch(url.replace(':userId', userId))
            ...
            .then(tasks => {
                let tasksHtml = '<table class="table table-bordered"><thead><tr><th>#</th><th>Tanggal</th><th>Status</th><th>Tugas</th><th>Main Proyek</th><th>Data Proyek</th><th>Dibuat</th><th>Ditugaskan</th><th>Action</th></tr></thead><tbody>';
                tasks.forEach(task => {
                    console.log(task);
