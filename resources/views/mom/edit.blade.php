                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select class="form-select select2" id="project_id" name="project_id" required onchange="loadMeetingsFromProject(this)">
                                    ...
                                    @foreach($projects as $project)
                                        <option value="{{ $project->id }}" data-meetings='@json($project->meetings_json)' {{ $project->id == $mom->project_id ? 'selected' : '' }} data-meetingselected='{{ $mom->meeting_id }}'>
                                            {{ $project->title }}
                                        </option>
                                    @endforeach
                                </select>
...
<script>
    ...
    function showMeetingInfo() {
        ...
        if (meetingSelect.value && selectedOption.dataset.participants) {
            ...
            console.log("kslmdakmdlka");
            
            if (participants.length > 0) {
                let participantsHtml = '';
                participants.forEach(participant => {
                    console.log("dasa,sald,");
