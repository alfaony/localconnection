                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <input type="date" name="date" class="form-control" id="date" value="{{ old('date') ?? @$workOrder->date ?? \Carbon\Carbon::today()->format('Y-m-d') }}" required>
                            </div>
                        </div>
                        <div class="row mt-2">
    ...
    function addForm(defaultProductId = null, price = null, defaultQty = 1,defaultDescription = null) 
    {
        ...
        $.each(groupedProducts, function (category, products) {
            ...
            products.forEach(function (product) {
                ...
                if(product.deleted_at && product.id == defaultProductId)
                {
                    projectOptions += `<option value="${product.id}" data-key="${key}" ${isSelected}>${product.name}</option>`;
                }
            });
