            ...
            <form method="post" action="{{ route('bast.update',$bast) }}">
                ...
                <form method="post" action="{{ route('bast.store') }}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    <input type="date" name="date" class="form-control" id="date"
                                        value="{{ old('date') ?? @$bast->date ?? \Carbon\Carbon::today()->format('Y-m-d') }}"
                                        required>
                                </div>
                            </div>
                            <div class="row mt-2">
                    ...
                    <select class="form-control select2" name="work_order" id="pilihSPK" required>
                        ...
                        <option value="{{ $a->id }}"
                        {{ @$bast->work_order_id == $a->id ? 'selected' : '' }}>{{ $a->number_result }}</option>
                        @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 projectChange" name="project" id="" required>
                            ...
                            <option value="{{ $a->id }}" {{ $a->reportProject ? '' : 'disabled' }}
                                data-statusreport="{{ $a->reportProject ? $a->reportProject->is_approve : '' }}"
                                data-report="{{ $a->reportProject ? $a->reportProject->id : '' }}"
                                {{ @$bast->project_id == $a->id ? 'selected' : '' }}
                                {{ @$selectedWorkOrder->id == $a->work_order_id ? 'selected' : '' }}>{{ $a->title }} -
                                {{ $a->workOrder->number_result }}</option>
                            @endforeach
                            <!-- Other options can be added here -->
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <button type="button" id="requestReportButton" class="btn btn-warning mt-2 text-white"
                            style="display: none;"><i class="fa fa-exclamation-circle" aria-hidden="true"></i>
                            Request</button>
                        @endcanAccess
                    </div>
                    <hr>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="period" id="period" placeholder="Periode"
                            value="{{ old('periode') ?? @$bast->period }}">
                    </div>
                    <div class="form-group">
                        <label for="purchaseOrder" class="form-label">No. Purchase Order</label>
                        <input type="text" class="form-control" name="number_purchase" id="purchaseOrder"
                            placeholder="PO 048392003" value="{{ old('number_purchase') ?? @$bast->number_purchase  }}"
                            required>
                    </div>
                    <div class="form-group">
                        <label for="penanggungJawab" class="form-label">Penanggung Jawab</label>
                        <input type="text" class="form-control" name="pic" id="penanggungJawab"
                            placeholder="Susi Susanti" value="{{ old('pic') ?? @$bast->pic  }}" required>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="customer_signature" id="signature" required>
                            ...
                            <option value="{{ $a }}" {{ @$bast->customer_signature == $a ? 'selected' : '' }}>
                                {{ $value }}</option>
                            @endforeach
                            <!-- Other options can be added here -->
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="template" required>
                            ...
                            <option value="{{ $a }}" {{ @$bast->template == $a ? 'selected' : '' }}>{{ $value }}
                            </option>
                            @endforeach
                            <!-- Other options can be added here -->
                        </select>
                    ...
                    <div class="form-group">
                        <label for="signature">Table Signature</label>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="enableTableSignature"
                                name="enableTableSignature" {{ !empty(@$bast->signature) ? 'checked' : '' }}>
                            <label class="form-check-label" for="enableTableSignature">
                                Enable Table Signature
                            </label>
                        <div id="tableSignatureContainer"
                            style="{{ !empty(@$bast->signature) ? 'display: block;' : 'display: none;' }}; margin-top: 10px;">
                            <div id="signatureInputs">
                                @if (!empty(@$bast->signature))
                                @foreach (json_decode($bast->signature_data, true) as $index => $signature)
                                <div class="row signature-row mb-2">
                                    <div class="col-md-5">
                                        <input type="text" name="table_signature[{{ $index }}][section_name]"
                                            class="form-control" placeholder="Nama Section"
                                            value="{{ $signature['section_name'] ?? '' }}">
                                    </div>
                                    <div class="col-md-5">
                                        <input type="text" name="table_signature[{{ $index }}][pic_name]"
                                            class="form-control" placeholder="Nama PIC"
                                            value="{{ $signature['pic_name'] ?? '' }}">
                                    </div>
                                    <div class="col-md-2">
                                        <button type="button" class="btn btn-danger remove-signature-row"><i
                            ...
                            <button type="button" id="addSignatureRow" class="btn btn-success mt-2"><i
...
$(document).ready(function() {
    ...
    // Show/Hide Table Signature Container
     $('#enableTableSignature').on('change', function () {
        if ($(this).is(':checked')) {
            $('#tableSignatureContainer').show();
            if ($('#signatureInputs .signature-row').length === 0) {
                console.log("hereee");
            ...
            $('#signatureInputs').empty(); // Hapus semua input jika tidak dicentang
        }
    });

    // Add Signature Row
    $('#addSignatureRow').on('click', function () {
        const rowCount = $('#signatureInputs .signature-row').length;
        if (rowCount >= MAX_ROWS) {
            Swal.fire('Error', `You can only add up to ${MAX_ROWS} rows.`, 'warning');
            return;
        }
        addSignatureRow(rowCount);
    });
    ...
    function addSignatureRow() {
        ...
        const index = $('#signatureInputs .signature-row').length;
        const newRow = `
                <div class="row signature-row mb-2">
                    <div class="col-md-5">
                        <input type="text" name="table_signature[${index}][section_name]" class="form-control" placeholder="Nama Section" required>
                    </div>
                    <div class="col-md-5">
                        <input type="text" name="table_signature[${index}][pic_name]" class="form-control" placeholder="Nama PIC" required>
                    </div>
                    <div class="col-md-2">
                        <button type="button" class="btn btn-danger remove-signature-row"><i class="fa fa-trash"></i></button>
                    </div>
                </div>
            `;
        $('#signatureInputs').append(newRow);
    }

    // Update Input Names (after deletion)
    function updateInputNames() {
        $('#signatureInputs .signature-row').each(function(index, row) {
            $(row).find('input[name*="[section_name]"]').attr('name',
                `table_signature[${index}][section_name]`);
            $(row).find('input[name*="[pic_name]"]').attr('name',
            `table_signature[${index}][pic_name]`);
        });
    }

    // Validate Before Submit
    $('form').on('submit', function(e) {
        if ($('#enableTableSignature').is(':checked')) {
            const rowCount = $('#signatureInputs .signature-row').length;
    ...
    if ($('#enableTableSignature').is(':checked')) 
    {
        const existingRows = $('#signatureInputs .signature-row').length;
        if (existingRows === 0) {
            addSignatureRow(0); // Tambahkan baris pertama jika belum ada data
        }
    }
...
$(document).ready(function() {
    ...
    $('.projectChange').on('change', function() {
        ...
        if (!reportId || statusReport == 0) {
            ...
            $('#saveButtonId').prop('disabled',
                true); // diasumsikan bahwa tombol simpan memiliki id 'saveButtonId'
            if (projectId) {
                $('#requestReportButton').show().data('project-id',
                    projectId); // Menampilkan tombol request dan menyimpan Project ID
            }
        } else {
    ...
    $('#requestReportButton').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                Swal.fire('Request Sent', 'Your request has been successfully sent.',
                    'success');
            },
            error: function(xhr) {
